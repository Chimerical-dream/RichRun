...
namespace Game.Core {
    ...
    public class VersionValidator {
        ...
        private void BackupOldSave() {
            ...
            var projectPathForSaveBackups = Path.Combine(Application.dataPath, "SaveBackups", _curSave.GameVersion + " " + SaveSystem.FileName);
            if (File.Exists(projectPathForSaveBackups) == false) { // don't override old file by new one with the same version if exists
                SaveSystem.SaveData(_curSave, projectPathForSaveBackups, new SaveBinarySerializer());
            ...
            SaveSystem.SaveData(_curSave, path, new SaveJSONSerializer()); // JSON to read data by key if save is broken
        }
