    ...
    public class CustomGameObjectCreateMenu {
        ...
        private static void CreateCanvasIfNoYet(GameObject elementToCheck) {
            if (elementToCheck.GetComponentInParent<Canvas>(includeInactive: true) == null) {
                var canvas = CreateGameObject(CanvasObjectTemplate, "Canvas", selectObject: false);
        ...
        private static GameObject CreateGameObject(string templateName, string endObjectName = "", PrefabUnpackMode unpackMode = PrefabUnpackMode.Completely, bool selectObject = true) {
