...
namespace Game.Core
{
    ...
    public class CameraConstantWidth : MonoBehaviour
    {
        ...
        [InfoBox("Should we change FOV if screen is wider then reference resolution")]
        [SerializeField] private bool _useOnWideScreens = false;

        [InfoBox("Leave 0 to make constant width\n" +
        ...
        private void Start()
        {
            ...
            if (componentCamera)
            {
                if (componentCamera.m_Lens.Orthographic)
                {
                    ...
                    float constantWidthSize = initialSize * (_targetAspect / componentCamera.m_Lens.Aspect);
                ...
                {
                    _initialFov = componentCamera.m_Lens.FieldOfView;
                    _horizontalFov = CalcVerticalFov(_initialFov, 1 / _targetAspect);

                    float constantWidthFov = CalcVerticalFov(_horizontalFov, componentCamera.m_Lens.Aspect);
                    if (constantWidthFov > componentCamera.m_Lens.FieldOfView || _useOnWideScreens)
                    {
                        componentCamera.m_Lens.FieldOfView = Mathf.Lerp(constantWidthFov, _initialFov, WidthOrHeight);
                    }
                }
            }
            else
